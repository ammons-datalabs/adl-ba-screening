...
public sealed class AzureMapsGeocodingService(HttpClient httpClient, IOptions<AzureMapsOptions> options) : IGeocodingService
{
    ...
    public async Task<GeocodingResult> GeocodeAsync(string address, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var url = $"{_options.BaseUrl}/search/address/json?api-version=1.0&subscription-key={_options.SubscriptionKey}&query={encodedAddress}";
